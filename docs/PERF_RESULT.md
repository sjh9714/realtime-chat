...
분리한다. 이 결과도 local 단일 앱 시나리오 검증이며 production, multi-instance, 1,000-session mixed
...
fan-out 검증으로만 해석합니다. production mixed traffic p95와 production benchmark는 별도 측정 대상입니다.
...
**한계:** 실제 트래픽은 읽기(채팅방 목록, 메시지 이력)와 쓰기(메시지 전송, 읽음 처리)가 혼합된다. local 시나리오 검증은 있지만 production send-to-receive end-to-end latency, 수신 completeness, 메시지 순서 정확도 수치는 아직 없다.

**개선 방향:** `k6/mixed-chat-test.js`의 local smoke와 10-room/50-user mixed HTTP probe repeat3는
통과했지만, production RPS/p95와 cache hit ratio를 공개 benchmark로 올리려면 더 큰 환경과 반복 실행을
별도로 기록해야 한다.
