...
public class ChatMessageProducer {
  ...
  public CompletableFuture<SendResult<String, Object>> sendMessage(ChatMessageEvent event) {
    ...
    future.whenComplete(
        (result, ex) -> {
          if (ex != null) {
            log.error(
                "메시지 발행 실패: roomId={}, messageKey={}",
                event.getRoomId(),
                event.getMessageKey(),
                ex);
          } else {
            log.debug(
                "메시지 발행 성공: roomId={}, messageKey={}, partition={}, offset={}",
                event.getRoomId(),
                event.getMessageKey(),
  ...
  public void sendReadReceipt(ReadReceiptEvent event) {
    String key = String.valueOf(event.getRoomId());
    kafkaTemplate
        .send(KafkaConfig.READ_RECEIPTS_TOPIC, key, event)
        .whenComplete(
            (result, ex) -> {
              if (ex != null) {
                log.error(
                    "읽음 처리 발행 실패: roomId={}, userId={}", event.getRoomId(), event.getUserId(), ex);
              }
            });
  }
}
