...
class ChatRoomIntegrationTest extends BaseIntegrationTest {
  ...
  void chatRoomFlow() {
    ...
    ResponseEntity<ChatRoomResponse> directResponse =
    ...
    ResponseEntity<ChatRoomResponse> duplicateResponse =
        postWithAuth("/api/rooms/direct", directRequest, ChatRoomResponse.class, token1);
    ...
    ResponseEntity<ChatRoomResponse> groupResponse =
        postWithAuth("/api/rooms/group", groupRequest, ChatRoomResponse.class, token1);
    ...
    // 4. 내 채팅방 목록 조회
    ResponseEntity<List<ChatRoomListResponse>> listResponse =
  ...
  private <T> ResponseEntity<T> getListWithAuth(
      String path, ParameterizedTypeReference<T> responseType, String token) {
    HttpHeaders headers = new HttpHeaders();
    headers.setBearerAuth(token);
    return restTemplate.exchange(
        baseUrl + path, HttpMethod.GET, new HttpEntity<>(headers), responseType);
