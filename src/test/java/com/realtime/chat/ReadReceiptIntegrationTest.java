...
class ReadReceiptIntegrationTest extends BaseIntegrationTest {
    ...
    void ownMessagesAreExcludedFromUnreadCount() {
        ...
        messageRepository.saveAndFlush(new Message(UUID.randomUUID(), room, user1, "내 메시지", MessageType.TEXT));
        messageRepository.saveAndFlush(new Message(UUID.randomUUID(), room, user2, "상대 메시지", MessageType.TEXT));

        readReceiptService.processReadReceipt(new ReadReceiptEvent(room.getId(), user1.getId(), 0L, LocalDateTime.now()));

        ChatRoomMember member = chatRoomMemberRepository.findByChatRoomIdAndUserId(room.getId(), user1.getId()).get();
    ...
    void messagesBeforeJoinedAtAreExcludedFromUnreadCount() {
        ...
        messageRepository.saveAndFlush(new Message(UUID.randomUUID(), room, owner, "참여 전 메시지", MessageType.TEXT));
        room.addMember(lateMember);
        chatRoomRepository.saveAndFlush(room);
        messageRepository.saveAndFlush(new Message(UUID.randomUUID(), room, owner, "참여 후 메시지", MessageType.TEXT));

        readReceiptService.processReadReceipt(new ReadReceiptEvent(room.getId(), lateMember.getId(), 0L, LocalDateTime.now()));

        ChatRoomMember member = chatRoomMemberRepository.findByChatRoomIdAndUserId(room.getId(), lateMember.getId()).get();
        assertThat(member.getUnreadCount()).isEqualTo(1);
    }
