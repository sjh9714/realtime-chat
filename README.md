...
<p align="center">
  <img src="docs/message-flow.svg" alt="Realtime Chat message flow" width="900">
</p>

- `CONNECT`는 JWT 인증 후 STOMP `Principal`에 `userId`를 바인딩합니다.
- `/topic/room.{roomId}` 구독과 `/app/chat.send` 전송은 room membership을 각각 검증합니다.
- 메시지는 `roomId` key로 Kafka에 publish되고, persistence와 broadcast consumer group이 독립적으로 처리합니다.
- consumer 실패 메시지는 DLT에 격리되며, 원인 제거 후 `DltReplayService`로 수동 replay할 수 있습니다.
