...
<p align="center">
  <img src="docs/architecture-overview.svg" alt="Realtime Chat system architecture overview" width="900">
</p>

| Step | Trigger | 처리 | 의미 |
|---|---|---|---|
| 1 | `CONNECT /ws` | JWT 검증 후 STOMP `Principal`에 `userId` 바인딩 | 인증된 WebSocket session 생성 |
| 2 | `SUBSCRIBE /topic/room.{roomId}` | room membership 검증 | `roomId` 추측 구독 차단 |
| 3 | `SEND /app/chat.send` | room membership 재검증 후 Kafka publish, key = `roomId` | 같은 room 메시지를 같은 partition으로 라우팅 |
| 4 | Kafka publish callback | `/user/queue/messages/ack` 또는 `/user/queue/messages/error` 응답 | Kafka broker accepted/failed 결과만 전달 |
| 5 | Kafka consumers | persistence group은 PostgreSQL 저장, broadcast group은 Redis Pub/Sub 발행 | 저장과 브로드캐스트 처리 경로 분리 |
| 6 | Consumer failure | DLT 격리 후 `DltReplayService` manual replay | 자동 복구가 아닌 수동 재처리 utility |

ACK/NACK는 Kafka publish 결과만 의미하며 DB persistence나 recipient delivery 완료를 뜻하지 않습니다. DLT replay는 자동화된 운영 복구가 아니라 원인 제거 후 수동으로 호출하는 내부 utility입니다.
