...
| 시나리오 검증 | Receiver matrix 500-user local repeat3 | 3회 모두 expected 49,900 / unique 49,900 / missing 0 / duplicate 0, p95 37-47ms, p99 46-233ms | local Docker Compose 반복 실행, 운영 성능 claim 아님 |
| 시나리오 검증 | Receiver matrix 1,000-user local repeat3 | 3회 모두 expected 99,900 / unique 99,900 / missing 0 / duplicate 0, p95 45-50ms, persisted message id ordering out-of-order 0 | local Docker Compose 반복 실행, mixed traffic 또는 운영 성능 claim 아님 |
| 시나리오 검증 | Mixed HTTP probe 10-room/50-user repeat3 | 3회 모두 expected 4,900 / unique 4,900 / missing 0 / duplicate 0, receiver p95 18-20ms, HTTP failed 0/90 | local 단일 앱 반복 실행, production/multi-instance/cache hit claim 아님 |
...
| 추가 측정 예정 | production 1,000 session send-to-receive latency | benchmark 미측정 | local receiver matrix repeat3는 시나리오 검증이며 production/mixed benchmark가 아님 |
| 추가 측정 예정 | production 1,000 session delivery completeness | benchmark 미측정 | local receiver matrix repeat3는 시나리오 검증이며 production/mixed benchmark가 아님 |
| 추가 측정 예정 | production / multi-instance mixed traffic p95 latency | benchmark 미측정 | 10-room/50-user mixed HTTP probe repeat3는 local 단일 앱 시나리오 검증 |
| 추가 측정 예정 | production / mixed 1,000-session benchmark | benchmark 미측정 | local receiver matrix repeat3는 [`docs/WEBSOCKET_MEASUREMENT.md`](docs/WEBSOCKET_MEASUREMENT.md)에 시나리오 검증으로 분리 |
